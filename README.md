...
Dieses GitHub-Repository benutzt synthetische Daten, die Syntax und Semantik des Kerndatensatz der Medizinformatik-Initiative abbilden (mehr dazu [hier](https://www.medizininformatik-initiative.de/de/der-kerndatensatz-der-medizininformatik-initiative)). Sie stellen Gesundheitsdaten aus der stationären Versorgung dar. Andere Datenquellen, wie z.B. Wetterdaten, stellen offene Daten dar, die weitere Informationen zu beispielsweise Umweltfaktoren der Patient:innen liefern.

Die synthetischen Daten der MII-Patient:innen sind in [data/raw/](data/raw/) zu finden. Eine Übersicht zu benutzten (externen) Daten folgt in [Tabelle 1](#benutzte-datenquellen).
...
1. **Erstellung der DCAT-AP.de-Metadaten**:  
   ...
   - Unter **Verteilung** (`dcat:distribution`) Zugangs-URLs und Dateiformate beschrieben, um Nutzern einen direkten Zugriff zu ermöglichen.
...
Die benutzten Datenquellen und die synthetischen MII-Daten arbeiten mit verschiedenen geographischen Angaben, wie Adresse und Postleitzahl, oder Koordinaten (Breitengrad/ Längengrad). Es wurden daher Skripte erstellt, die helfen zwischen den Angaben zu mappen. Skripte für geographisches Mapping sind unter [address_transformation.py](src/geolocation/address_transformation.py) zu finden.
